...
- 在屏幕右上角显示电量，连续两分钟无操作后自动关闭背光；
- 显示地址二维码，或通过 USB 键盘模式输入地址；
- 将钱包名称、地址、zpub、指纹和派生路径保存到 SD 卡。

`/card-wallets.txt` 仅包含公开信息，不能用于恢复资金。只有用户明确选择“加密备份到 SD”时，秘密材料才会写入 `/bw-vault.dat`；保险库密码不会保存到设备或 SD 卡。
...
启动或选择“加载钱包”时，BW 会从设备 NVS 读取上次使用的钱包文件路径，并直接加载这一份钱包索引文件，不再打开通用文件浏览器；索引路径失效时会自动尝试 `/card-wallets.txt`。新保险库密码至少 12 个字符；为兼容旧备份，旧保险库仍允许原有 8 字符密码解锁。解锁失败不会回退到明文数据。
...
6. 将签名文件导回桌面钱包，或直接扫描动态二维码，然后再次检查并广播。

加密保险库和 RFID 标签都不是纸质备份的替代品。不要在 SD 卡上另存明文助记词文件。
签名前必须以 Cardputer 屏幕显示的交易详情为准；地址、金额或手续费不符合预期时立即取消。

连续两分钟没有按键操作时，屏幕会自动关闭以节省电量。此时按任意有效按键只会唤醒屏幕，不会同时执行原界面操作；松开后再按一次即可继续。
